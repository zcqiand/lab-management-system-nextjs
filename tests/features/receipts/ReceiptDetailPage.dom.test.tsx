...
const {
  ...
  testRecordTable,
  paramInterfaceTable,
  inspectionParameterParamInterfaceTable,
} = tablesOf();
...
describe("接样单详情页", () => {
  ...
  fnTest(
    ...
    async () => {
      ...
      // 检测依据：第二条标准带名称
      expect(screen.getByText(/GB 175-2023 通用硅酸盐水泥/)).toBeTruthy();
      // 反向断言：旧行为只剩裸编码的行不应再出现
      const cellTexts = Array.from(document.querySelectorAll(".col-span-2")).map(
        (el) => el.textContent ?? "",
      );
