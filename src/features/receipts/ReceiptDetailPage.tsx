...
export function ReceiptDetailPage() {
  ...
  const paramLabels =
    receipt.testParameters?.map((c) => paramLabel(c, parameters)).join(", ") ?? "—";
    ...
    <div className="space-y-4" data-fn="M03.F09.I02">
      ...
      <div className="bg-white rounded shadow p-4">
        ...
        <div className="grid grid-cols-4 gap-x-4 gap-y-1 text-sm">
          {/* 委托书信息 */}
          <div>
            <span className="text-gray-500">委托书编号：</span>
            {receipt.commissionCode}
          </div>
          <div>
            <span className="text-gray-500">委托书登记号：</span>
            {receipt.commissionRegisterCode ?? "—"}
          </div>
          <div>
          ...
          <div>
            <span className="text-gray-500">委托书登记日期：</span>
            {receipt.commissionRegisterDate ?? "—"}
          </div>
          {/* 从合同带出 */}
          <div>
            <span className="text-gray-500">工程名称：</span>
            {receipt.projectName ?? "—"}
          </div>
          <div>
          ...
          <div>
            <span className="text-gray-500">建设单位：</span>
            {receipt.buildingUnit ?? "—"}
          </div>
          <div>
            <span className="text-gray-500">监理单位：</span>
          ...
          <div>
            <span className="text-gray-500">见证单位：</span>
            {receipt.witnessUnit ?? "—"}
          </div>
          <div>
          ...
          <div>
            <span className="text-gray-500">见证人电话：</span>
            {receipt.witnessPhone ?? "—"}
          </div>
          <div>
            <span className="text-gray-500">送检人：</span>
            {receipt.inspector ?? "—"}
          </div>
          <div>
            <span className="text-gray-500">送检人电话：</span>
            {receipt.inspectorPhone ?? "—"}
          </div>
          {/* 取样信息 */}
          <div>
            <span className="text-gray-500">取样地点：</span>
            {receipt.samplingLocation ?? "—"}
          </div>
          <div>
            <span className="text-gray-500">接样人：</span>
            {receipt.receivedBy}
          </div>
          {/* 报告类别 + 检测类别 */}
          <div>
            <span className="text-gray-500">报告类别：</span>
            {categoryLabel(receipt.categoryCode, reportNames)}
          </div>
          <div>
          ...
          <div>
            <span className="text-gray-500">样品来源：</span>
          ...
          <div className="col-span-2">
            <span className="text-gray-500">判定依据：</span>
            {standardLabels(receipt.judgmentBasis, standards)}
          </div>
          <div className="col-span-2">
            <span className="text-gray-500">检测依据：</span>
            {standardLabels(receipt.testingBasis, standards)}
          </div>
          <div className="col-span-4">
            <span className="text-gray-500">检测参数：</span>
            {paramLabels}
          </div>
          {/* 流程信息 */}
          <div>
            <span className="text-gray-500">流程状态：</span>
            {FLOW_STAGE_LABELS[receipt.flowStatus]}
          </div>
          <div>
            <span className="text-gray-500">检测结果：</span>
            {receipt.result ? (receipt.result === "pass" ? "合格" : "不合格") : "—"}
          </div>
          {receipt.reportCode && (
            <div>
          ...
          {receipt.assigneeName && (
            <div>
              <span className="text-gray-500">检测负责人：</span>
              {receipt.assigneeName}
            </div>
          )}
          {receipt.plannedTestDate && (
            <div>
              <span className="text-gray-500">计划检测日期：</span>
              {receipt.plannedTestDate}
            </div>
          )}
          {/* 检测信息（数据录入环节维护） */}
          <div>
            <span className="text-gray-500">检测环境：</span>
            {receipt.testEnvironment ?? "—"}
          </div>
          <div>
            <span className="text-gray-500">主要设备：</span>
            {receipt.mainEquipment ?? "—"}
          </div>
          <div>
            <span className="text-gray-500">检测人员：</span>
            {receipt.testOperator ?? "—"}
          </div>
          <div>
            <span className="text-gray-500">原始记录单号：</span>
            {receipt.originalRecordNo ?? "—"}
          </div>
          <div>
            <span className="text-gray-500">开始日期：</span>
            {receipt.testStartDate ?? "—"}
          </div>
          <div>
      ...
      {/* 样品信息 + 检测数据 */}
      <ReceiptDetail receiptId={receipt.id} categoryCode={receipt.categoryCode} />
