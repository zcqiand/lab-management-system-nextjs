...
interface Props<T extends TreeListItem> {
  /** 列表 API 路径：inspection-calculation-rules / inspection-technical-requirements */
  listEndpoint: string
...
export function TwoLevelObjectStandardTree<T extends TreeListItem>(props: Props<T>) {
  ...
  const selectedStandard = selectedStandardProp !== undefined ? selectedStandardProp : innerSelected
  ...
  const [standardsByObject, setStandardsByObject] = useState<Record<string, StandardNode[]>>({})
  ...
  useEffect(() => {
    apiClient
      .get<{ items: ObjectNode[] }>(route(objectsEndpoint), { params: { page: '1', pageSize: '500' } })
      .then((r) => {
      ...
      .then((res) => {
        ...
        if (dragEnabled) {
          ...
          items = items.filter((it) => {
            const k = sortKeys.map((key) => String((it as Record<string, unknown>)[key] ?? '')).join('|')
    ...
    <div className="flex flex-col flex-1 min-h-0" data-fn={dataFn}>
      ...
      <div className="grid grid-cols-[260px_1fr] gap-4 flex-1 min-h-0">
        ...
        <aside className="bg-white rounded shadow overflow-hidden flex flex-col min-h-0">
          <div className="px-3 py-2 text-xs font-semibold text-gray-500 bg-gray-50 border-b shrink-0">检测项目</div>
                ...
                <li key={o.code}>
                  ...
                  {isOpen && (
                    <ul className="bg-gray-50">
                      {stds.length === 0 && (
                        <li className="px-3 py-1.5 text-xs text-gray-400 pl-8">加载中...</li>
                      )}
                      {stds.map((s) => {
        ...
        <section className="bg-white rounded shadow overflow-hidden flex flex-col min-h-0">
          ...
          {list.length > 0 && (
            ...
            >
              <SortableContext items={list.map((i) => getItemId(i))} strategy={verticalListSortingStrategy}>
                <div className="flex-1 overflow-auto min-h-0">
                  ...
                  <div className="sticky top-0 z-10 flex items-center bg-gray-50 border-b px-3 py-2 text-xs font-semibold text-gray-600 min-w-max">
                    {dragEnabled && <span className="w-4 mr-2 shrink-0" aria-hidden="true" />}
...
function SortableRow<T extends TreeListItem>({
  ...
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: getItemId(item) })
  ...
  return (
    ...
    >
      ...
      <span className="w-12 text-center text-xs text-gray-500 tabular-nums shrink-0" data-testid={`sort-${id}`}>
        {item.sortOrder ?? '-'}
      </span>
      {columns.map((c, i) => {
      ...
      <div className="ml-auto pl-3 space-x-2 whitespace-nowrap shrink-0">
        {/* @entry editDataFn / deleteDataFn 由父组件传入：计算规则 M06.F05.I02/I03，技术要求 M06.F06.I02/I03 */}
        <button onClick={() => onEdit(item)} data-fn={editDataFn} className="px-2 py-1 text-blue-600 hover:underline">编辑</button>
        <button onClick={() => onDelete(item)} data-fn={deleteDataFn} className="px-2 py-1 text-red-600 hover:underline">删除</button>
      </div>
    </li>
  )
}
