...
describe("M01.F05.I07 SaasAuthClient", () => {
  ...
  it("401 maps to UnauthorizedClient", async () => {
    ...
    const calls = await client
      .token("authorization_code", "code", null, "http://lab.local/cb")
      .catch((e) => e);
  ...
  it("400 maps to InvalidGrant", async () => {
    ...
    const calls = await client
      .token("authorization_code", "bad", null, "http://lab.local/cb")
      .catch((e) => e);
    expect(calls).toBeInstanceOf(SaasAuthError);
  ...
  it("construct validates required env", () => {
    expect(
      () =>
        new HttpSaasAuthClient({
          baseUrl: "",
          clientId: "x",
          clientSecret: "y",
          defaultTenantId: "z",
          callbacks: { redirectUri: "c" },
        }),
    ).toThrow(/baseUrl/);
    expect(
      () =>
