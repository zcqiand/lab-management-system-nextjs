...
describe("InspectionCapabilityPage M06 CRUD 入口", () => {
  ...
  fnTest(
    ...
    async () => {
      ...
      // B6 加载态：整页 PageLoading 门控后，搜索框随数据一起出现 → findBy 断言
      await user.type(await screen.findByLabelText("搜索"), "IP-FORM-1");
  ...
  fnTest(
    ["M06.F01.I05"],
    "AssociationManager 行内前缀：检测标准性质以【】呈现",
    async () => {
      render(
        <AssociationManager
  ...
  fnTest(["M06.F02.I01"], "检测项目清单聚合显示检测参数名", async () => {
    ...
    expect(
      (await screen.findAllByText((t) => t.includes("凝结时间"))).length,
    ).toBeGreaterThan(0);
  });
  ...
  describe("畸形响应防御", () => {
    ...
    it("apiClient.get 返回空对象时不抛错，落入 error/empty 分支", async () => {
      ...
      // 渲染本身不得抛错
      expect(() =>
        render(<InspectionCapabilityPage resource="specialties" />),
      ).not.toThrow();
