...
3. **infra 副作用**：DB-First pull 链（`scripts/pull-schema.sh` → `src/db/schema.ts` 入 git）
   + pg devDep 借链 host（shared replay 测试借本仓 node_modules/pg）
...
- **DB-First（ADR-0025/0033）**：DB 真源在 shared `src/db/schema.ts`；本仓 `src/db/schema.ts`
  是 `bash scripts/pull-schema.sh` 从真库 pull 的产物，**入 git**；改库 → shared 改 schema →
  migrate → 本仓重 pull + commit。禁手写/手改 src/db/schema.ts 里的表结构
- **禁提交 `generated/`**（gitignored；`npm run gen:shared` 重出）
- **禁 `pg` 升 dependencies**（shared replay 测试借链不能进消费方 runtime bundle）
...
- SSOT（DB） → `../lab-management-system-shared/src/db/schema.ts`（本仓 pull + commit）；
  契约（API） → 同仓 `generated/openapi/`
