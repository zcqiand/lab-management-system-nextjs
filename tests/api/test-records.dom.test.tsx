...
describe("test-records API (M03.F03.I08 / I09)", () => {
  fnTest(
    ["M03.F03.I08"],
    "GET /api/test-records 按 sampleId 过滤 + 分页结构 {items,page,pageSize,total}",
    async () => {
      ...
      const seeded = ((await anyList.json()) as { items: Array<{ sampleId: string }> })
        .items;
      expect(seeded.length).toBeGreaterThan(0);
      const sampleId = seeded[0]!.sampleId;

      const res = await GET(
        req(`http://test/api/test-records?sampleId=${encodeURIComponent(sampleId)}`),
      );
  ...
  fnTest(
    ["M03.F03.I09"],
    "POST /api/test-records 创建后返回 201 带 TR- 前缀 id，GET 立即可见；清理不留种子",
    async () => {
