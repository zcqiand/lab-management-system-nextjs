...
describe("EntryModal 数据录入 3 锚点（M03.F03.I02/I04/I06）", () => {
  fnTest(["M03.F03.I02"], "footer「保存」按钮挂 M03.F03.I02 + 点击触发 PUT", async () => {
    ...
    await waitFor(() => expect(screen.getAllByText("S-001").length).toBeGreaterThan(0));
  ...
  fnTest(
    ["M03.F03.I10"],
    "侧栏样品卡右上角「×」按钮挂删除锚点 + 点击弹 ConfirmModal",
    async () => {
      const { container } = render(
        <EntryModal receipt={MOCK_RECEIPT} onClose={() => {}} />,
      );
      ...
      await waitFor(() => expect(screen.getByText(/删除样品 —/)).toBeInTheDocument());
    },
  );

  fnTest(
    ["M03.F03.I11"],
    "footer「改判」select 挂改判锚点 + 选 pass 后应用",
    async () => {
      let putCalled = false;
      server.use(
