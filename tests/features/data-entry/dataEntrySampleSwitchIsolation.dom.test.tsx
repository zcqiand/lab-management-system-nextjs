...
describe("DataEntryPage 切换样品 dirty 缓冲隔离", () => {
  ...
  fnTest(["M03.F03.I03"], "S1 输入破坏荷载 1 试件后切到 S2，S2 不被脏写", async () => {
    const user = userEvent.setup();
    render(<EntryModal receipt={buildReceiptWithTwoSamples()} onClose={() => {}} />);
  ...
  fnTest(["M03.F03.I03"], "S1 保存后切回 S1 看不到 dirty 值（已落库清掉）", async () => {
    const user = userEvent.setup();
    render(<EntryModal receipt={buildReceiptWithTwoSamples()} onClose={() => {}} />);
    await waitFor(() => {
      expect(screen.queryByText(/抗压强度代表值/)).toBeInTheDocument();
