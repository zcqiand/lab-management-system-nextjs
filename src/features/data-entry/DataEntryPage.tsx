...
export function EntryModal({
  ...
  // 参数 → 计算方法（取 specimenCount 驱动录入卡组数）。
  ...
  for (const cr of calcRules) {
    ...
    if (!cur) {
      calcRuleByParam.set(cr.inspectionParameterCode, {
        specimenCount: cr.specimenCount,
      });
      continue;
    }
              ...
              {selectedSample && (
                <>
                  <div className="text-xs text-gray-500 leading-relaxed">
                    ...
                    {selectedSample.manufacturer &&
                      ` / 厂家 ${selectedSample.manufacturer}`}
                    {selectedSample.structuralPart &&
                      ` / 部位 ${selectedSample.structuralPart}`}
                    ...
                    {selectedSample.batchNumber &&
                      ` / 批号 ${selectedSample.batchNumber}`}
                    {selectedSample.supplyUnit && ` / 供销 ${selectedSample.supplyUnit}`}
                    {selectedSample.arrivalDate &&
                      ` / 进场 ${selectedSample.arrivalDate}`}
                    {selectedSample.samplingDate &&
                      ` / 取样 ${selectedSample.samplingDate}`}
        ...
        message={
          <div className="text-sm">
            ...
            <p className="text-xs text-gray-500 mt-2">
              级联删除 {records.filter((r) => r.sampleId === deleteTarget?.id).length} 条
              testRecord。
            </p>
          </div>
        }
