// M06.F08 参数界面：GET/PUT/DELETE /api/param-interfaces/:id（id 或 code 皆可命中；
...
function findRow(id: string): Record<string, unknown> | undefined {
  return (paramInterfaces as unknown as Record<string, unknown>[]).find(
    (r) => r["id"] === id || r["code"] === id,
  );
}
...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!row) return notFound("ParamInterface not found");
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  const arr = paramInterfaces as unknown as Record<string, unknown>[];
