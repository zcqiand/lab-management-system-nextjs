...
describe(
  ...
  () => {
    ...
    it("list all：id=code 补列 + 命中种子行", async () => {
      const r = await listDictDb(DICT_CFGS.specialties, {
        page: 1,
        pageSizeParam: "1000",
      });
    ...
    it("keyword：code/name 字面子串命中", async () => {
      const byCode = await listDictDb(DICT_CFGS.specialties, {
        keyword: MARKER,
        page: 1,
      ...
      const byName = await listDictDb(DICT_CFGS.parameters, {
        keyword: "字典测试参数B",
        page: 1,
      });
    ...
    it("junction 反查链：fixture wrapDict 同款「存在链接」语义（query 值被忽略）", async () => {
      ...
      const byObj = await listDictDb(DICT_CFGS.parameters, {
      ...
      const bySp = await listDictDb(DICT_CFGS.parameters, {
      ...
      const stds = await listDictDb(DICT_CFGS.standards, {
        direct: { inspectionObjectCode: OBJ },
        page: 1,
        pageSizeParam: "1000",
      });
    ...
    it("聚合列：name 兜底 + '，' join + 去重保序", async () => {
      const objs = await listDictDb(DICT_CFGS.objects, { keyword: MARKER, page: 1 });
      ...
      const params = await listDictDb(DICT_CFGS.parameters, { keyword: MARKER, page: 1 });
    ...
    it("catalog：inspectionObjectCode 直列过滤 + tenant 隔离", async () => {
      const r = await listDictDb(CATALOG_CFGS.brands, { keyword: MARKER, page: 1 });
      ...
      const byObj = await listDictDb(CATALOG_CFGS.brands, {
        direct: { inspectionObjectCode: OBJ },
        page: 1,
      });
    ...
    it("POST：重复 code 返回 fixture 版原句 400 语义；新码创建成功", async () => {
      const dup = await createDictDb(DICT_CFGS.specialties, { code: SP, name: "重复" });
      expect(dup.ok).toBe(false);
      if (!dup.ok) expect(dup.message).toBe("专项编码已存在");
      const created = await createDictDb(DICT_CFGS.specialties, {
        code: `${MARKER}SP-NEW`,
        name: "PG字典测试专项2",
        officialNo: "九",
      });
      ...
      const after = await getDictDb(DICT_CFGS.specialties, `${MARKER}SP-NEW`);
      expect(after).toBeDefined();
    });

    it("PUT/DELETE roundtrip：body 覆盖 + code 不可改；删后 404 语义", async () => {
      const before = await getDictDb(DICT_CFGS.specialties, SP);
      expect(before).toBeDefined();
      const updated = await putDictDb(DICT_CFGS.specialties, SP, {
        name: "PG字典测试专项-改",
        code: "HACK-TRY",
      });
      ...
      // 未知键无列归宿静默丢弃（不炸）
      const withJunk = await putDictDb(DICT_CFGS.specialties, SP, { nonExistentCol: 1 });
      ...
      expect(await deleteDictDb(DICT_CFGS.specialties, SP)).toBe(true);
      expect(await getDictDb(DICT_CFGS.specialties, SP)).toBeUndefined();
      expect(await deleteDictDb(DICT_CFGS.specialties, SP)).toBe(false);
      // PUT 不存在的 code → undefined（路由层映射 404）
      expect(await putDictDb(DICT_CFGS.specialties, SP, { name: "x" })).toBeUndefined();
      // 还原种子行（其他断言可能复用）
      const s = await connect();
      await s`insert into inspection_specialties (code, official_no, name, sort_order, created_at, updated_at)
        values (${SP}, ${MARKER + "off"}, ${"PG字典测试专项"}, 9999, ${NOW_STAMP}, ${NOW_STAMP})`;
