import { pgTable, serial, text, bigint, index, uniqueIndex, foreignKey, integer, boolean, jsonb, timestamp, primaryKey } from "drizzle-orm/pg-core"
...
export const sampleReceipts = pgTable("sample_receipts", {
	...
	return {
		...
		idxSampleReceiptsFlowStatus: index("idx_sample_receipts_flow_status").using("btree", table.flowStatus.asc().nullsLast().op("text_ops")),
...
export const inspectionCalculationMethods = pgTable("inspection_calculation_methods", {
	inspectionObjectCode: text("inspection_object_code").notNull(),
	inspectionParameterCode: text("inspection_parameter_code").notNull(),
	...
	return {
		idxCalcMethodObject: index("idx_calc_method_object").using("btree", table.inspectionObjectCode.asc().nullsLast().op("text_ops")),
