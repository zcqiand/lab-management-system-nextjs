    ...
    try {
      ...
      setTechReqs(
        (Array.isArray(reqRes)
          ? reqRes
          : []) as unknown as InspectionTechnicalRequirement[],
      );
        ...
        <>
          <div>
            ...
            <div className="grid grid-cols-4 gap-x-4 gap-y-1 text-sm">
              ...
              <div>
                <span className="text-gray-500">牌号：</span>
                {activeSample.brand ?? "—"}
              </div>
              {/* 生产信息 */}
              <div>
                <span className="text-gray-500">出厂编号/批号：</span>
                {activeSample.batchNumber ?? "—"}
              </div>
              <div>
                <span className="text-gray-500">生产厂家/产地：</span>
                {activeSample.manufacturer ?? "—"}
              </div>
              <div>
                <span className="text-gray-500">供销单位：</span>
                {activeSample.supplyUnit ?? "—"}
              </div>
              <div>
                <span className="text-gray-500">进场日期：</span>
                {activeSample.arrivalDate ?? "—"}
              </div>
              {/* 施工信息 */}
              <div>
                <span className="text-gray-500">结构部位：</span>
                {activeSample.structuralPart ?? "—"}
              </div>
              <div>
                <span className="text-gray-500">取（制）样日期：</span>
                {activeSample.samplingDate ?? "—"}
              </div>
              <div>
                <span className="text-gray-500">养护条件：</span>
                {activeSample.curingCondition ?? "—"}
              </div>
              <div>
                <span className="text-gray-500">龄期：</span>
                {activeSample.age ?? "—"}
              </div>
              {/* 数量 */}
              <div>
                <span className="text-gray-500">代表数量：</span>
                {activeSample.representQuantity ?? "—"}
              </div>
              <div>
                <span className="text-gray-500">样品数量：</span>
                {activeSample.sampleQuantity ?? "—"}
              </div>
              {/* 备注 */}
              {activeSample.remark && (
            ...
            {activeItems.length === 0 ? (
              ...
              <div className="space-y-3">
                {activeItems.map((rec) => {
                  ...
                  if (!p) {
                    p = (
                      generatedParameters as Array<{
                        code: string;
                        name: string;
                        canonicalName?: string;
                        unit?: string;
                      }>
                    ).find((x) => x.code === rec.parameterCode) as
                  ...
                  const basisOptions = stdParams
                    ...
                    .map((sp) =>
                      standards.find((s) => s.code === sp.inspectionStandardCode),
                    )
