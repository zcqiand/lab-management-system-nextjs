...
export async function GET(_req: NextRequest, { params }: { params: { code: string } }) {
  try {
    const r = await getDictDb(DICT_CFGS.objects, params.code);
...
export async function PUT(req: NextRequest, { params }: { params: { code: string } }) {
  ...
  try {
    const r = await putDictDb(DICT_CFGS.objects, params.code, body);
    if (!r) return notFound("Object not found");
    return Response.json(r);
  } catch (e) {
    if (isDbUnavailable(e)) return dbUnavailable();
...
export async function DELETE(
  ...
  try {
    const ok = await deleteDictDb(DICT_CFGS.objects, params.code);
