...
const req = (
  ...
  ({
    id: "req-1",
    inspectionParameterCode: "IP",
    judgmentStandardCode: "GB",
    valueType: "numeric",
    comparison: "≥",
    minValue: 400,
...
describe("RebarMechNumericCard 抗拉/屈服强度", () => {
  it("填直径 + 最大力 → 抗拉强度 R=4000F/(πd²) 落 onChange", () => {
    ...
    fireEvent.change(screen.getByLabelText("第 1 组 最大力 (kN)"), {
      target: { value: "100" },
    });
...
describe("RebarMechNumericCard passthrough（伸长率）", () => {
  it("直接录入值 → 均值", () => {
    ...
    fireEvent.change(screen.getByLabelText("第 1 组 断后伸长率 (%)"), {
      target: { value: "18" },
    });
    fireEvent.change(screen.getByLabelText("第 2 组 断后伸长率 (%)"), {
      target: { value: "20" },
    });
    const last = onChange.mock.calls.at(-1)![0];
    const result = JSON.parse(last.result);
...
describe("RebarMechNumericCard readOnly", () => {
  it("只读 → 输入禁用", () => {
    ...
    expect(
      (screen.getByLabelText("第 1 组 最大力 (kN)") as HTMLInputElement).readOnly,
    ).toBe(true);
  });
});
