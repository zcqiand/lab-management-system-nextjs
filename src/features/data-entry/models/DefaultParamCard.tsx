...
export function DefaultParamCard({
  ...
  const passLabel = (r?: TestRecord) => {
    ...
    if (r.verdict === "合格" || r.verdict === "符合")
      return <span className="text-green-600">{r.verdict}</span>;
    if (r.verdict === "不合格" || r.verdict === "不符合")
      return <span className="text-red-600">{r.verdict}</span>;
    ...
    <div className="border rounded p-3 space-y-2">
      ...
      <div className="grid grid-cols-4 gap-x-3 gap-y-1 text-xs">
        ...
        <div>
            ...
            onChange={(e) =>
              onChange({
                ...
                requirement: requirementLabel(
                  reqOptions.find((r) => r.id === e.target.value) ?? reqOptions[0]!,
                ),
              })
            }
            disabled={readOnly}
          ...
          >
            <option value="">
              {rec?.requirement && !reqVal ? rec.requirement : "—"}
            </option>
            {reqOptions.map((r) => (
