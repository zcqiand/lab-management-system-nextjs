...
describe(
  ...
  () => {
    ...
    it("聚合列：name 兜底 + '，' join + 去重保序", async () => {
      const objs = await listDictDb(TENANT, DICT_CFGS.objects, {
        keyword: MARKER,
        page: 1,
      });
      ...
      const params = await listDictDb(TENANT, DICT_CFGS.parameters, {
        keyword: MARKER,
        page: 1,
      });
    ...
    it("catalog：inspectionObjectCode 直列过滤 + tenant 隔离", async () => {
      const r = await listDictDb(TENANT, CATALOG_CFGS.brands, {
        keyword: MARKER,
        page: 1,
      });
    ...
    it("POST：重复 code 返回 fixture 版原句 400 语义；新码创建成功", async () => {
      const dup = await createDictDb(TENANT, DICT_CFGS.specialties, {
        code: SP,
        name: "重复",
      });
    ...
    it("PUT/DELETE roundtrip：body 覆盖 + code 不可改；删后 404 语义", async () => {
      ...
      // 未知键无列归宿静默丢弃（不炸）
      const withJunk = await putDictDb(TENANT, DICT_CFGS.specialties, SP, {
        nonExistentCol: 1,
      });
      ...
      expect(
        await putDictDb(TENANT, DICT_CFGS.specialties, SP, { name: "x" }),
      ).toBeUndefined();
      // 还原种子行（其他断言可能复用）
      const s = await connect();
      await s`insert into inspection_specialties (code, official_no, name, sort_order, created_at, updated_at)
