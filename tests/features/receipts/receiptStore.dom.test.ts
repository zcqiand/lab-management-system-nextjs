...
describe("receiptStore 状态流转", () => {
  ...
  fnTest(["M03.F01.I01"], "fetchReceipts 支持 keyword 搜索", async () => {
    ...
    await useReceiptStore
      .getState()
      .fetchReceipts({ page: 1, pageSize: 10, keyword: "XYZ" });
    const s = useReceiptStore.getState();
  ...
  fnTest(["M03.F01.I01"], "fetchReceipts 支持 categoryCode 过滤", async () => {
    ...
    await useReceiptStore
      .getState()
      .fetchReceipts({ page: 1, pageSize: 10, categoryCode: "cement" });
    const s = useReceiptStore.getState();
    expect(s.list).toHaveLength(1);
  ...
  fnTest(["M03.F01.I03"], "updateReceipt 成功", async () => {
    ...
    await useReceiptStore
      .getState()
      .updateReceipt(target.id, { receivedBy: "李四", remark: "复检" });
    const s = useReceiptStore.getState();
  ...
  fnTest(
    ["M03.F01.I03"],
    "updateReceipt 不存在时 upsert 由 msw 404 → error（REF 语义差异见任务报告）",
    async () => {
      // msw PUT /receipts/:id 对不存在 id 返回 404（REF 旧 msw 是 upsert）。
      // store 行为：error 填充、不抛出——断言 error 非空即「不报错地失败」。
      await useReceiptStore
        .getState()
        .updateReceipt("rc-upsert-test", { receivedBy: "测试员" });
