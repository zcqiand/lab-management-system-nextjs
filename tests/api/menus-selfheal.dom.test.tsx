...
describe("/api/auth/menus miss 自愈", () => {
  ...
  function mockSaasOk(menuTree: unknown[] = []): void {
    fetchMock = vi.fn(async (url: unknown) => {
      ...
      if (u.includes("/api/v1/me/menus")) {
        return new Response(JSON.stringify({ "lab-management": menuTree }), {
          status: 200,
          headers: { "content-type": "application/json" },
        });
      }
  ...
  fnTest(
    ["M01.F04.I04"],
    "miss + saas 登录失败 → 仍 503 MENUS_UNAVAILABLE（错误语义不变）",
    async () => {
      fetchMock = vi.fn(async (url: unknown) => {
        const u = String(url);
        if (u.includes("/api/v1/auth/login")) {
  ...
  fnTest(
    ["M01.F04.I04"],
    "无 Authorization 返 401（ADR-0019 删 demo USER-A 兜底）",
    async () => {
  ...
  fnTest(
    ["M01.F04.I04"],
    "自愈成功后再次 miss 应走缓存命中——不重复调 saas serviceLogin（限流兜底）",
    async () => {
      mockSaasOk([
        {
          id: "m-dash",
