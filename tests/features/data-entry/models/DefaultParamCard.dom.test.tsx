...
function baseProps(overrides: Partial<ParamModelProps> = {}): ParamModelProps {
  return {
    parameter: {
      ...
      canonicalName: "含水率",
      unit: "%",
    } as ParamModelProps["parameter"],
    record: undefined,
    sampleId: "s-test",
    standards: [
      { code: "GB/T 1", name: "示例标准" } as ParamModelProps["standards"][number],
    ],
    stdParams: [
      {
        inspectionParameterCode: "IP-0001",
        inspectionStandardCode: "GB/T 1",
      } as ParamModelProps["stdParams"][number],
    ],
    techReqs: [],
    config: undefined,
    onChange: vi.fn(),
...
describe("DefaultParamCard", () => {
  ...
  it("录入检测结果时 onChange 上报 {result}", async () => {
    ...
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({ result: expect.stringContaining("12.3") }),
    );
  });
