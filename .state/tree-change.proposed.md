...
| M06.F05 | 计算方法 | CalculationRule 维护（复合主键，算法类型 + 公式） | 接口 | 规划 |
...
| M01.F04.I04 | 动态菜单 | 接口 | 侧边栏菜单由身份平台 GET /menus?appId=lab-management 下发，按权限码显隐；分组无可见子项则隐藏 | 规划 |
...
| 子项 ID | 名称 | 类型 | 说明 | 状态 |
|---|---|---|---|---|
| M02.F01.I01 | 合同列表 | 接口 | 合同 CRUD | 规划 |
| M02.F01.I02 | 合同新建/编辑 | 接口 | 合同信息维护 | 规划 |
| M02.F01.I03 | 合同删除 | 接口 | 合同删除 | 规划 |
| M02.F01.I04 | 合同汇总 | 接口 | 按合同统计汇总数据 | 已废弃 |


| 子项 ID | 名称 | 类型 | 说明 | 状态 |
|---|---|---|---|---|
| M03.F01.I01 | 接样单列表 | 接口 | 创建/查看/编辑/删除接样单，含 categoryCode | 规划 |
| M03.F01.I02 | 接样单新建 | 接口 | 含 receiptCode/contractId/categoryCode | 规划 |
| M03.F01.I03 | 接样单编辑 | 接口 | 接样单信息修改 | 规划 |
| M03.F01.I04 | 接样单删除 | 接口 | 接样单删除 | 规划 |
| M03.F01.I05 | 接样单详情 | 接口 | 含 flowStatus（待提交/审核中/已批准/已发放/已归档）+ flowHistory | 已废弃 |
| M03.F01.I06 | 接样单三态过滤器 | 接口 | 全部/未提交/已提交：按 flowStatus 过滤接样单列表 | 规划 |
| M03.F01.I07 | 接样单 ext 字段补录 | 接口 | 报告预览前按当前类别 extFields 弹 SampleExtFieldsModal，补录持久化到 Sample.ext | 规划 |


| 子项 ID | 名称 | 类型 | 说明 | 状态 |
|---|---|---|---|---|
| M03.F02.I01 | 任务分配 | 接口 | 接样提交后进入分配中，安排检测人员+计划日期 | 规划 |
| M03.F02.I02 | 任务编辑 | 接口 | 安排弹窗维护 assigneeName/assigneeId/plannedTestDate | 规划 |
| M03.F02.I03 | 任务取消（清空分配） | 接口 | 清空 assignee/assigneeId/plannedTestDate，把已分配单子在本阶段重置为未分配（非退回接样；退回接样走 FlowStagePage 通用退回按钮） | 规划 |
| M03.F02.I04 | 任务分配三态过滤器 | 接口 | 全部/未提交/已提交：按 flowStatus 过滤任务分配列表 | 规划 |


| 子项 ID | 名称 | 类型 | 说明 | 状态 |
|---|---|---|---|---|
| M03.F03.I01 | 数据录入页面 | 接口 | 样品检测数据录入 | 规划 |
| M03.F03.I02 | 检测数据保存 | 接口 | 保存录入的检测数据 | 规划 |
| M03.F03.I03 | 检测项编辑 | 接口 | 编辑已有检测数据 | 规划 |
| M03.F03.I04 | 检测项删除 | 接口 | 删除检测项记录 | 规划 |
| M03.F03.I05 | 自动评定 | 接口 | 系统按技术要求自动判定合格/不合格（TestRecord 精简后已移除自动评定，改为人工 verdict） | 已废弃 |
| M03.F03.I06 | 人工改判 | 接口 | 手工修正自动评定结果 | 规划 |
| M03.F03.I07 | 数据录入三态过滤器 | 接口 | 全部/未提交/已提交：按 flowStatus 过滤数据录入列表 | 规划 |


| 子项 ID | 名称 | 类型 | 说明 | 状态 |
|---|---|---|---|---|
| M03.F05.I01 | 报告审核页面 | 接口 | 报告审核列表 | 规划 |
| M03.F05.I02 | 报告查看 | 接口 | 查看报告详情 | 规划 |
| M03.F05.I03 | 审核操作 | 接口 | 审核通过/驳回 | 规划 |
| M03.F05.I04 | 报告审核三态过滤器 | 接口 | 全部/未提交/已提交：按 flowStatus 过滤报告审核列表 | 规划 |


| 子项 ID | 名称 | 类型 | 说明 | 状态 |
|---|---|---|---|---|
| M03.F06.I01 | 报告批准页面 | 接口 | 报告批准列表 | 规划 |
| M03.F06.I02 | 报告查看 | 接口 | 查看报告详情 | 规划 |
| M03.F06.I03 | 批准操作 | 接口 | 批准通过/驳回 | 规划 |
| M03.F06.I04 | 报告批准三态过滤器 | 接口 | 全部/未提交/已提交：按 flowStatus 过滤报告批准列表 | 规划 |


| 子项 ID | 名称 | 类型 | 说明 | 状态 |
|---|---|---|---|---|
| M03.F07.I01 | 报告发放页面 | 接口 | 报告发放列表 | 规划 |
| M03.F07.I02 | 报告查看 | 接口 | 查看报告详情 | 规划 |
| M03.F07.I03 | 发放操作 | 接口 | 报告发放 | 规划 |
| M03.F07.I04 | 报告发放三态过滤器 | 接口 | 全部/未提交/已提交：按 flowStatus 过滤报告发放列表 | 规划 |


| 子项 ID | 名称 | 类型 | 说明 | 状态 |
|---|---|---|---|---|
| M03.F08.I01 | 报告归档页面 | 接口 | 报告归档列表 | 规划 |
| M03.F08.I02 | 报告查看 | 接口 | 查看报告详情 | 规划 |
| M03.F08.I03 | 归档操作 | 接口 | 报告归档 | 规划 |
| M03.F08.I04 | 报告归档三态过滤器 | 接口 | 全部/未提交/已提交：按 flowStatus 过滤报告归档列表 | 规划 |


| 子项 ID | 名称 | 类型 | 说明 | 状态 |
|---|---|---|---|---|
| M03.F09.I01 | 查看按钮 | 接口 | 各流程列表页 rowActions 加查看按钮，触发详情页 | 规划 |
| M03.F09.I02 | 详情页 | 接口 | 展示接样信息、样品列表、检测数据；检测参数显示为「名称(单位)」、报告类别显示为报告简称 | 规划 |
| M03.F09.I03 | 报告预览（详情页） | 接口 | 详情页标题栏按钮，复用 ReportPreviewModal，按 receipt.categoryCode 找模板 docx 渲染 | 规划 |


| 子项 ID | 名称 | 类型 | 说明 | 状态 |
|---|---|---|---|---|
| M04.F06.I01 | 型号列表 | 接口 | InspectionModel 实体码表，左侧检测项目树 + 右侧列表（拖拽调整 sortOrder 持久化） | 规划 |
| M04.F06.I02 | 型号新建/编辑 | 接口 | 型号码表维护 | 规划 |
| M04.F06.I03 | 型号删除 | 接口 | 删除保护：被技术要求引用时拒绝 | 规划 |


| 子项 ID | 名称 | 类型 | 说明 | 状态 |
|---|---|---|---|---|
| M04.F07.I01 | 规格列表 | 接口 | InspectionSpec 实体码表，左侧检测项目树 + 右侧列表（拖拽调整 sortOrder 持久化） | 规划 |
| M04.F07.I02 | 规格新建/编辑 | 接口 | 规格码表维护 | 规划 |
| M04.F07.I03 | 规格删除 | 接口 | 删除保护：被技术要求引用时拒绝 | 规划 |


| 子项 ID | 名称 | 类型 | 说明 | 状态 |
|---|---|---|---|---|
| M04.F08.I01 | 等级列表 | 接口 | InspectionGrade 实体码表，左侧检测项目树 + 右侧列表（拖拽调整 sortOrder 持久化） | 规划 |
| M04.F08.I02 | 等级新建/编辑 | 接口 | 等级码表维护 | 规划 |
| M04.F08.I03 | 等级删除 | 接口 | 删除保护：被技术要求引用时拒绝 | 规划 |


| 子项 ID | 名称 | 类型 | 说明 | 状态 |
|---|---|---|---|---|
| M04.F09.I01 | 牌号列表 | 接口 | InspectionBrand 实体码表，左侧检测项目树 + 右侧列表（拖拽调整 sortOrder 持久化） | 规划 |
| M04.F09.I02 | 牌号新建/编辑 | 接口 | 牌号码表维护 | 规划 |
| M04.F09.I03 | 牌号删除 | 接口 | 删除保护：被技术要求引用时拒绝 | 规划 |
...
| 子项 ID | 名称 | 类型 | 说明 | 状态 |
|---|---|---|---|---|
| M06.F05.I01 | 计算方法列表 | 接口 | 2 级树（检测项目→检测标准）+ 右侧列表（拖拽调整 sortOrder）；列：算法类型/试件数/备注 | 规划 |
| M06.F05.I02 | 计算方法新建/编辑 | 接口 | 维护原始数据到检测结果的算法和适用条件 | 规划 |
| M06.F05.I03 | 计算方法删除 | 接口 | 删除未被检测数据引用的计算方法 | 规划 |
