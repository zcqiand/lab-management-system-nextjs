...
| M02.F01.I02 | 合同新建/编辑 | 接口 | 前端+后端 | 合同信息维护 | 已上线 |
| M02.F01.I03 | 合同删除 | 接口 | 前端+后端 | 合同删除 | 已上线 |
...
| 子项 ID | 名称 | 类型 | 交付 | 说明 | 状态 |
|---|---|---|---|---|---|
| M03.F03.I01 | 数据录入页面 | 接口 | 前端+后端 | 样品检测数据录入 | 已上线 |
| M03.F03.I02 | 检测数据保存 | 接口 | 前端+后端 | 保存录入的检测数据 | 已上线 |
| M03.F03.I03 | 检测项编辑 | 接口 | 前端+后端 | 编辑已有检测数据 | 已上线 |
| M03.F03.I04 | 检测项删除 | 接口 | 前端+后端 | 删除检测项记录 | 已上线 |
| M03.F03.I05 | 自动评定 | 接口 | 前端+后端 | 系统按技术要求自动判定合格/不合格（TestRecord 精简后已移除自动评定，改为人工 verdict） | 已废弃 |
| M03.F03.I06 | 人工改判 | 接口 | 前端+后端 | 手工修正自动评定结果 | 已上线 |
...
| M04.F06.I03 | 型号删除 | 接口 | 前端+后端 | 删除保护：被技术要求引用时拒绝 | 已上线 |
...
| M04.F07.I03 | 规格删除 | 接口 | 前端+后端 | 删除保护：被技术要求引用时拒绝 | 已上线 |
...
| M04.F08.I03 | 等级删除 | 接口 | 前端+后端 | 删除保护：被技术要求引用时拒绝 | 已上线 |
...
| M04.F09.I03 | 牌号删除 | 接口 | 前端+后端 | 删除保护：被技术要求引用时拒绝 | 已上线 |
...
| M06.F01.I03 | 检测专项删除 | 接口 | 前端+后端 | 删除自定义且未被引用的专项，保护官方及已引用专项 | 已上线 |
...
| M06.F02.I03 | 检测项目删除 | 接口 | 前端+后端 | 删除自定义且未被引用的项目，保护官方及已引用项目 | 规划 |
| M06.F02.I04 | 关联检测依据 | 接口 | 前端+后端 | 维护 role=TESTING 的项目检测依据标准 | 已上线 |
| M06.F02.I05 | 关联判定依据 | 接口 | 前端+后端 | 维护 role=JUDGMENT 的项目判定依据标准 | 已上线 |
| M06.F02.I06 | 关联检测参数 | 接口 | 前端+后端 | 维护项目参数多对多及资质必备/可选；prefilter 已上线（先选检测项目→过滤参数，UX 见 REQ-2026-011） | 已上线 |
| M06.F02.I07 | 关联检测专项 | 接口 | 前端+后端 | 维护 InspectionSpecialtyObject 多对多 | 已上线 |
...
| M06.F03.I03 | 检测参数删除 | 接口 | 前端+后端 | 删除自定义且未被关联的参数，保护官方及已引用参数 | 已上线 |
...
| M06.F04.I03 | 检测标准删除 | 接口 | 前端+后端 | 删除自定义且未被引用的标准，保护已引用标准 | 规划 |
| M06.F04.I04 | 关联检测参数 | 接口 | 前端+后端 | 维护标准参数多对多及条款、方法、单位、试验规则；prefilter 弹窗（先选检测项目→过滤参数）见 REQ-2026-011 | 已上线 |
...
| M06.F05.I03 | 计算方法删除 | 接口 | 前端+后端 | 删除未被检测数据引用的计算方法 | 已上线 |
...
| M06.F06.I03 | 技术要求删除 | 接口 | 前端+后端 | 删除未被检测结果引用的技术要求 | 已上线 |
...
| M06.F07.I03 | 报告名称删除 | 接口 | 前端+后端 | 删除保护：被关联项目/标准/参数引用时拒绝 | 已上线 |
| M06.F07.I04 | 关联检测项目 | 接口 | 前端+后端 | 维护 InspectionObjectReportName 中间表 | 已上线 |
| M06.F07.I05 | 关联检测依据 | 接口 | 前端+后端 | 维护 InspectionReportNameStandard role=TESTING | 已上线 |
| M06.F07.I06 | 关联判定依据 | 接口 | 前端+后端 | 维护 InspectionReportNameStandard role=JUDGMENT | 已上线 |
| M06.F07.I07 | 关联检测参数 | 接口 | 前端+后端 | 维护 InspectionReportNameParameter 中间表；prefilter 弹窗（先选检测项目→过滤参数）+ 参数清单行追加「· 对象名」，见 REQ-2026-012 | 已上线 |
| M06.F07.I08 | 扩展属性维护 | 接口 | 前端+后端 | 维护 InspectionReportName.extFields 列表（key/label/type/required/tag/source/options）；预览前若样品 ext 未覆盖对应 key，会自动弹 SampleExtFieldsModal（M03.F01.I07）让用户补录 | 已上线 |
