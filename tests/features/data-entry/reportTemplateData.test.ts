...
describe("assembleReport", () => {
  ...
  it("mock 回退：无 TestRecord 时 jcz 仍非空", () => {
    ...
    if ("items" in out) expect(out.items.every((i) => i.jcz.length > 0)).toBe(true);
  });
  ...
  it("试件轴：IP-0055 result JSON 解析后每个样品 1 行，kyqd0/1/2 各 1 试件 + dbz 共享", () => {
    ...
    const allText = out.rows
      .map((r) => r.kyqd0 + "|" + r.kyqd1 + "|" + r.kyqd2 + "|" + r.dbz)
      .join("\n");
...
describe("flattenForDocx", () => {
  it("RN-105-1 抗压走 grid 模式（flattenForDocx 走 cell 展开；不再原样返回 rows）", () => {
    ...
    const flat = flattenForDocx("RN-105-1", "105_混凝土抗压强度检测报告", out, [
      sample("s1"),
  ...
  it("RN-101 grid：扁平字典含 manifest tag + 通用表头 + 样品摘要；param 回退非空", () => {
    ...
    const flat = flattenForDocx("RN-101", "101_水泥检测报告", out, [
      sample("s1"),
  ...
  it("mock 源确定性", () => {
    ...
    const a = flattenForDocx("RN-101", "101_水泥检测报告", out, [sample("s1")]) as Record<
      string,
      string
    >;
    const b = flattenForDocx("RN-101", "101_水泥检测报告", out, [sample("s1")]) as Record<
      string,
      string
    >;
