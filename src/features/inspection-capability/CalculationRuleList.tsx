...
// REF src/features/inspection-capability/CalculationRuleList.tsx 移植。
// 差异：apiClient → @/api/legacy-client + API_ROUTES。msw 计算规则主键是
...
export function CalculationRuleList() {
  ...
  const save = async () => {
    ...
    try {
      if (editId) await apiClient.put(`${API_ROUTES['/inspection-calculation-rules']}/${editId}`, payload)
      else await apiClient.post(API_ROUTES['/inspection-calculation-rules'], payload)
      setOpen(false)
  ...
  const remove = async (id: string) => {
    ...
    await apiClient.delete(`${API_ROUTES['/inspection-calculation-rules']}/${id}`)
    if (selectedStandard) reloadList(selectedStandard)
  }
  ...
  return (
    <div data-fn="M06.F05.I01" className="flex flex-col flex-1 min-h-0">
      {/* @entry M06.F05.I02 计算规则新建/编辑按钮（TwoLevelObjectStandardTree 内"新建"+"编辑"） */}
      {/* @entry M06.F05.I03 计算规则删除按钮（TwoLevelObjectStandardTree 内行"删除"） */}
      <TwoLevelObjectStandardTree<CalcRow>
        ...
        columns={[
          ...
          { key: 'specimenCount', label: '上限', width: 'w-16', align: 'right', render: (it) => '×' + it.specimenCount },
        ...
        message={
          <div className="space-y-3 text-left text-sm">
            ...
            <label className="block">
              ...
              <input aria-label="检测标准" value={form.testingStandardCode} onChange={(e) => setForm({ ...form, testingStandardCode: e.target.value })} className="mt-1 w-full border rounded px-2 py-1.5 font-mono" placeholder="如 GB/T 228.1-2021" />
            </label>
            <label className="block">
