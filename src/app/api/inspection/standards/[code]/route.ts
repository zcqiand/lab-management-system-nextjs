// M06.F04 检测标准：PUT/DELETE /api/inspection/standards/:code
...
export async function PUT(req: NextRequest, { params }: { params: { code: string } }) {
  ...
  try {
    const r = await putDictDb(auth.tenantId, DICT_CFGS.standards, params.code, body);
...
export async function DELETE(
  ...
  if (auth instanceof NextResponse) return auth;
  try {
    const ok = await deleteDictDb(auth.tenantId, DICT_CFGS.standards, params.code);
