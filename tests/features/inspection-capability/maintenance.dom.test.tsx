...
describe("M06.F05 计算方法维护", () => {
  fnTest(
    ["M06.F05.I02"],
    "选标准后新建按钮开弹窗：检测项目/参数/标准/算法/试件数字段齐备",
    async () => {
      render(<CalculationMethodList />);
      // seed 链路：OBJ-SP01-P5（混凝土及拌合用水）→ GB/T 50081-2019 有 20 条计算方法
      await selectStandard("object-OBJ-SP01-P5", "GB/T 50081-2019");
      const user = userEvent.setup();
      await user.click(screen.getByRole("button", { name: "新建" }));
      expect(await screen.findByText("新建计算方法")).toBeTruthy();
...
describe("M06.F07 报告名称维护面", () => {
  fnTest(
    ["M06.F07.I02"],
    "新建按钮开 5 页签编辑弹窗（基础/项目/标准/参数/扩展属性）",
    async () => {
  ...
  fnTest(
    ["M06.F07.I04", "M06.F07.I05", "M06.F07.I06", "M06.F07.I07"],
    "编辑弹窗 4 关联页签可达：项目/标准(检测+判定)/参数",
    async () => {
      render(<ReportNameList />);
      await flush();
      const user = userEvent.setup();
...
describe("M06.F08 参数界面维护面", () => {
  ...
  fnTest(
    ["M06.F08.I02"],
    "新建按钮开弹窗：code/name/componentPath/config 字段齐备",
    async () => {
      render(<ParamInterfaceList />);
      await flush();
      const user = userEvent.setup();
  ...
  fnTest(
    ["M06.F08.I05", "M06.F08.I06"],
    "行预览按钮开只读预览弹窗（挂 I06 data-fn）",
    async () => {
      render(<ParamInterfaceList />);
      await flush();
      await waitFor(() => {
