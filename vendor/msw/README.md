...
建筑工程实验室管理系统的 mock 边界 —— MSW v2 handler + 跨前端一致 fixture，独立 HTTP 后端（`:5200`）。
...
npm run dev           # 起 HTTP server，监听 :5200（健康检查 /healthz）
npm start             # 生产路径
```
...
- 确定性 fixture（CRUD/流程在内存持久），跨端一致，重启丢失（ADR-0012 B 强度，不持久化）
- 独立 HTTP 部署：`src/server.ts` listen `:5200`，供 react / vue / nextjs 前端直接 fetch
