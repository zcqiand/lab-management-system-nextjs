...
// 显式 $type<> 让 flowHistory 推断为 Array<...> 而非 unknown,
// 否则 src/app/api/audit-logs/route.ts:39 的 cast 报 type 不兼容。
type FlowEntry = {
  action: string;
  from: string;
  to: string;
...
export const sampleReceipts = pgTable("sample_receipts", {
  ...
  flowHistory: jsonb("flow_history").$type<FlowEntry[]>().default([]).notNull(),
});
