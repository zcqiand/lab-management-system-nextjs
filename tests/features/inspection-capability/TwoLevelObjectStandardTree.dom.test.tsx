...
//   '*/api/inspection/standards'，/inspection-calculation-rules -> '*/api/calculation-rules'，
...
const STANDARDS_BY_OBJECT: Record<string, never[]> = {
  ...
  concrete: [{ id: "s3", code: "GB/T 50081-2019", name: "GB/T 50081-2019", sortOrder: 1 } as never],
};
...
beforeEach(() => {
  ...
  server.use(
    http.get("*/api/inspection/objects", () =>
      HttpResponse.json({ items: OBJECTS, total: OBJECTS.length, page: 1, pageSize: 500 }),
    ),
  );
});
...
describe("TwoLevelObjectStandardTree 2 级树 + 拖拽列表", () => {
  fnTest(["M06.F05.I01"], "左树渲染检测项目", async () => {
    server.use(http.get("*/api/inspection/standards", () => HttpResponse.json({ items: [] })));
    server.use(http.get("*/api/calculation-rules", () => HttpResponse.json({ items: [] })));
    render(
      <TwoLevelObjectStandardTree<DemoItem>
        title="计算规则"
        listEndpoint="/inspection-calculation-rules"
        getItemId={(it) => it.id}
  ...
  fnTest(["M06.F05.I01"], "展开检测项目 -> 显示子级检测标准", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("standard-GB/T 228.1-2021")).toBeInTheDocument());
    expect(screen.getByTestId("standard-GB/T 232-2010")).toBeInTheDocument();
  });

  fnTest(["M06.F05.I01"], "选中检测标准 -> 右侧展示列表 + sortOrder + 拖拽把手", async () => {
    server.use(
      http.get("*/api/inspection/standards", () => HttpResponse.json({ items: STANDARDS_BY_OBJECT.steel })),
    );
    server.use(
      http.get("*/api/calculation-rules", () =>
        HttpResponse.json({
          items: [
            { id: "r1", algorithmType: "simple_avg", specimenCount: 3, sortOrder: 10, createdAt: "", updatedAt: "" },
            { id: "r2", algorithmType: "manual", specimenCount: 1, sortOrder: 20, createdAt: "", updatedAt: "" },
          ],
        }),
      ),
    );
  ...
  fnTest(["M06.F06.I01"], "技术要求也复用：filterParam=judgmentStandardCode", async () => {
    server.use(
      http.get("*/api/inspection/standards", () => HttpResponse.json({ items: STANDARDS_BY_OBJECT.steel })),
    );
  ...
  fnTest(["M06.F05.I01"], "计算规则：点击标准 -> 用 testingStandardCode 过滤 GET", async () => {
    server.use(
      http.get("*/api/inspection/standards", () => HttpResponse.json({ items: STANDARDS_BY_OBJECT.steel })),
    );
    let lastFilter: string | null = null;
    ...
    server.use(
      http.get("*/api/calculation-rules", ({ request }) => {
        ...
        const allItems = [
          { id: "r1", algorithmType: "simple_avg", specimenCount: 3, testingStandardCode: "GB/T 228.1-2021", sortOrder: 10, createdAt: "", updatedAt: "" },
          { id: "r2", algorithmType: "manual", specimenCount: 1, testingStandardCode: "GB/T 232-2010", sortOrder: 20, createdAt: "", updatedAt: "" },
        ];
  ...
  fnTest(["M06.F05.I01"], "未选标准时右侧显示空态", async () => {
    ...
    await screen.findByTestId("object-steel");
    await waitFor(() => expect(screen.getByText("请先选择左侧检测项目下的检测标准")).toBeInTheDocument());
  });
});
